...
SKILL_MECHANICS: dict[str, list[str]] = {
    "SMALL_GUNS": ["手枪、冲锋枪、步枪的命中基础。完整命中公式见 [[战斗公式]]。", "[[Sniper 狙击手]] 要求小型枪械 80%，之后每次远程命中都有机会升级为暴击。"],
    "BIG_GUNS": ["迷你机枪、火箭筒、火焰喷射器的命中基础，见 [[战斗公式]]。", "这类武器多为连发或范围伤害：连发不吃瞄准的暴击加成，而且会沿弹道误伤队友。"],
    "ENERGY_WEAPONS": ["激光、等离子武器的命中基础，见 [[战斗公式]]。", "能量武器打的是激光、等离子伤害类型，对普通伤害抗性高的目标（动力装甲、机器人）往往更有效。"],
    "UNARMED": [
        "徒手攻击的命中基础，见 [[战斗公式]]。徒手伤害为 `随机(1, 近战伤害属性 + 2)`，攻击消耗 3 AP。",
        "[[Slayer 杀手]] 要求肉搏 80%，之后近战和徒手命中即暴击。",
    ],
    "MELEE_WEAPONS": ["刀、锤、矛等近战武器的命中基础，见 [[战斗公式]]。", "近战和徒手的瞄准惩罚只有远程的一半，是低命中角色打部位的划算方式。", "伤害为 `随机(武器最小值, 武器最大值 + 近战伤害属性)`。"],
    "THROWING": ["飞刀、手榴弹等投掷武器的命中基础，见 [[战斗公式]]。", "投掷最大射程 = 3 ×（力量 + 2 × [[Heave Ho! 飞刀手！]] 等级）（`item.cc`）。", "投掷武器**不加**近战伤害属性。"],
    "FIRST_AID": [
