...
PERK_EFFECTS: dict[str, dict] = {
    "AWARENESS": {"effect": ["查看（Examine）活着的生物时，额外显示对方的当前/最大生命值；若右手持武器，还显示武器名和剩余弹药（`protinst.cc`）。",
                             "另有 18 个角色或遭遇脚本检查此 Perk（如伊恩、伊佐、拉尔斯、索尔等），作用待挖。"]},
    ...
    "MENTAL_BLOCK": {"effect": ["只有 `REVULSE.INT` 检查此 Perk，推测与大师的精神干扰场有关，待挖。"]},
    ...
    "ANIMAL_FRIEND": {"effect": ["只在老鼠和辐射蝎相关脚本中检查（`RADRAT`、`RADSCORP`、`RADSCOR2`、`WANRATS`、`WANRAT2`），具体行为待挖。"]},
    ...
    "RANGER": {"effect": ["城市、海岸、沙漠、山地等 6 个随机遭遇脚本检查此 Perk，把结果存入脚本变量后续使用，具体如何降低敌对遭遇待挖。"]},
...
- 动物之友、野外专家、搜寻者、意志力、警觉力在脚本里的具体行为。
