...
项目不调用外部解包器、反编译器、图像或音频转换器处理游戏资源。当前使用 Python 标准库自行读取 DAT1、MSG、INT、FRM、PAL、MAP、PRO、LST 和 ACM，并自行编码标准索引 PNG 与 PCM WAV，以减少第三方可执行文件、覆盖行为和路径越界风险。解析器以二进制只读模式打开来源，并验证计数、偏移、大小、编号关系和格式边界。
...
所有实际写入继续由本项目自身代码完成。第三方资料用于交叉核对已观察到的格式；ACM 算法的 MIT 来源和许可文本见 `docs/third-party-notices.md`。项目不引入第三方二进制或运行依赖。
